...
public class DownloadZipFileServlet extends HttpServlet {
    ...
    /**
     *
     * Upon receiving the path of the temporary directory where the result
     * file(s) are stored, a zip file will be created, where all the results
     * will be compressed for the user to download.
     *
     * @param request
     * @param response
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        for (String userTmpDir : UserDirs) {
            ...
            //If user directory is older than 2 days it will be deleted.
            if (dirDate > twoDays) {
                FileUtils.deleteDirectory(userDir);
            }
        }
        //Deletes the downloaded user directory
        FileUtils.deleteDirectory(directory);
    }

    /**
     * Compress the results file(s) into a zip file, if the filename starts with
     * "export".
     *
     * @param files
     * @return boas
    ...
    public byte[] zipFiles(File[] files) throws IOException {
        ...
        //Go through all the subdirectories of the user 
        for (File dirName : files) {

            //Takes the subdirectory that starts with "analysis" which contain 
